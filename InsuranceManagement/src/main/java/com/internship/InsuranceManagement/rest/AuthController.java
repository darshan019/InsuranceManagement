...
public class AuthController {
    ...
    // Tries Admin, then Agent, then Customer. Returns JWT + role + userId.

    // Frontend reads the role from the response and redirects accordingly.
    ...
    public ResponseEntity<?> unifiedLogin(@RequestBody LoginRequest loginRequest) {
        ...
        if (!admins.isEmpty()) {
            ...
            loginAuditService.logEvent("ADMIN", admin.getAdminId(), admin.getEmail(), "LOGIN");
        ...
        List<Agent> agents = entityManager.createQuery(
        ...
        if (!agents.isEmpty()) {
            ...
            loginAuditService.logEvent("AGENT", agent.getAgentId(), agent.getEmail(), "LOGIN");
        ...
        // 3. Try Customer (BCrypt-hashed password)
        ...
        if (!customers.isEmpty()) {
            ...
            loginAuditService.logEvent("CUSTOMER", customer.getCustomerId(), customer.getEmail(), "LOGIN");
    ...
    // Legacy per-role endpoints (kept so existing clients don't break)
    ...
    private int getUserIdByRoleAndEmail(String role, String email) {
        ...
        if ("ADMIN".equals(role)) {
            ...
            return entityManager.createQuery(
                            ...
                            "SELECT a.adminId FROM Admin a WHERE a.email = :email", Integer.class)
        ...
        if ("AGENT".equals(role)) {
            ...
            return entityManager.createQuery(
                            ...
                            "SELECT a.agentId FROM Agent a WHERE a.email = :email", Integer.class)
        ...
        return entityManager.createQuery(
                        ...
                        "SELECT c.customerId FROM Customer c WHERE c.email = :email", Integer.class)
    ...
    public ResponseEntity<String> logout(@RequestHeader("Authorization") String authHeader) {
        ...
        loginAuditService.logEvent(role, userId, email, "LOGOUT");
