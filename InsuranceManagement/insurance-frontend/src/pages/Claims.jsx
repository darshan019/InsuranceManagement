  ...
  const loadClaims = async () => {
    ...
    try {
      ...
      if (role === 'CUSTOMER') {
        ...
        // Fetch this customer's claims from the DB (filtered by customer_id),

        // NOT from localStorage.

        const res = await api.get('/api/claims/customer/' + user.userId);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const payload = {
        ...
        // Attach the logged-in customer so customer_id is written to the DB.

        // (AGENT-filed claims won't have user.userId as a customer; the backend

        // falls back to the policy's customer in that case.)

        ...(role === 'CUSTOMER' && user && user.userId

          ? { customer: { customerId: parseInt(user.userId) } }
